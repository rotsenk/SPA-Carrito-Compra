...
> Si agregamos muchos productos, en el `aside` cuando presionamos al carrito, se esconde el botón de limpiar, pantalla, podemos solucionar eso, colocando `style={{ maxHeight: '500px', overflow: 'auto' }}` en el `aside` dentro de `ul` que renderiza los productos que agregamos al carrito
